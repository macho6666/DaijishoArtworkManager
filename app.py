...
class App(tk.Tk):
    ...
    def _ui(self):
        ...
        ttk.Button(controls,text="웹 이미지 검색",command=self.google_browser).pack(side="left")
        ttk.Button(controls,text="후보 이미지 보기",command=self.google_api_search).pack(side="left",padx=4)
    ...
    def list_remote_dirs(self, path):
        q = self.shell_quote(path.rstrip("/") or "/")
        cmd = f'for f in {q}/*; do [ -d "$f" ] && basename "$f"; done'
        out = self.runadb(["shell", "sh", "-c", cmd])
        return sorted([x.rstrip("\\r") for x in out.splitlines() if x.strip()], key=str.casefold)

    def path_settings(self):
        try:
            self.check_adb()
            start = self.rom_path.get().strip() or "/sdcard"
    ...
    def load_roms(self):
        try:
            ...
            out = self.runadb(["shell", "ls", "-1", rom])
            names = [x.rstrip("\\r") for x in out.splitlines() if x.strip()]
            # Keep files only, so subfolders are not treated as ROMs.
            file_names=[]
            for n in names:
                full=posixpath.join(rom,n)
                q=self.shell_quote(full)
                check=self.runadb(["shell","sh","-c",f'[ -f {q} ] && echo FILE'],check=False)
                if "FILE" in check:
                    file_names.append(n)
    ...
    def api_settings(self):
        messagebox.showinfo(
            "v0.4",
            "v0.4부터 후보 이미지 보기는 별도 Google API 설정 없이 동작합니다.\n"
            "검색이 막히거나 원하는 표지가 없으면 '웹 이미지 검색' 또는 '내 이미지 선택'을 사용하세요."
        )

    def google_api_search(self):
        q=self.google_query()
        if not q:return messagebox.showinfo("선택","먼저 ROM을 선택하세요.")
        self.status.set("후보 이미지 검색 중...")
        threading.Thread(target=self._candidate_worker,args=(q,),daemon=True).start()

    def _candidate_worker(self,q):
        try:
            from urllib.parse import quote_plus
            # API-key-free candidate discovery. Bing's public image result page exposes murl values.
            url="https://www.bing.com/images/search?q="+quote_plus(q)+"&form=HDRSC3"
            r=requests.get(url,timeout=15,headers={"User-Agent":"Mozilla/5.0"})
            r.raise_for_status()
            urls=[]
            for u in re.findall(r'&quot;murl&quot;:&quot;(https?://.*?)(?:&quot;|")',r.text):
                u=u.replace("\\/","/").replace("&amp;","&")
                if u not in urls: urls.append(u)
                if len(urls)>=18: break
            data=[]
            for u in urls:
                try:
            ...
            self.after(0,lambda:messagebox.showerror("후보 검색 실패",f"{e}\n\n웹 이미지 검색 또는 내 이미지 선택을 사용해 주세요."))
        finally:
    ...
    def show_candidates(self,data):
        ...
        for idx,(im,url) in enumerate(data[:9]):
            ...
            ph=ImageTk.PhotoImage(thumb); self.search_photos.append(ph); self.search_urls.append(url)
        ...
        if not data:
            ttk.Label(self.candidates,text="후보를 찾지 못했습니다. 웹 이미지 검색 또는 내 이미지 선택을 사용하세요.").pack()
