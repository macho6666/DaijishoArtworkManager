import os, re, io, json, base64, shutil, subprocess, threading, webbrowser, sys
...
class App(tk.Tk):
    ...
    def _ui(self):
        ...
        ttk.Entry(top,textvariable=self.rom_path,width=48).pack(side="left",fill="x",expand=True)
        ...
        ttk.Label(media,text="  (ROM 폴더명을 자동 사용 · 직접 수정 가능)").pack(side="left")
    ...
    def load_roms(self):
        try:
            ...
            # No extension whitelist. Every regular file in the selected folder is listed.
            # NUL-delimited output keeps spaces/special characters and separate files intact.
            quoted = "'" + rom.replace("'", "'\\\\''") + "'"
            shell_cmd = f'for f in {quoted}/*; do [ -f "$f" ] && printf "%s\\\\0" "$f"; done'
            p = subprocess.run(
                [self.adb(), "exec-out", "sh", "-c", shell_cmd],
                capture_output=True
            )
            if p.returncode:
                raise RuntimeError(p.stderr.decode("utf-8", "replace").strip())
            paths = [x.decode("utf-8", "replace") for x in p.stdout.split(b"\\0") if x]
            names = sorted([x.rsplit("/", 1)[-1] for x in paths], key=str.casefold)
            ...
            save_cfg(self.cfg)
            self.refresh_tree()
            self.status.set(f"{len(names)}개 파일")
        except Exception as e:
