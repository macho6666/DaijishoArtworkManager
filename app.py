...
class App(tk.Tk):
    ...
    def _google_candidate_worker(self,q):
        try:
            from urllib.parse import quote_plus
            # Google Images only. No Bing/game DB/API.
            ...
            # Google image pages commonly expose original image URLs in JSON/script data.
            found=[]
            patterns=[
            ...
            for pat in patterns:
                for u in re.findall(pat,html,flags=re.I):
                    u=(u.replace("\\u003d","=")
                         ...
                         .replace("\\/","/")
                         .replace("&amp;","&"))
                    if "gstatic.com/images" in u or "google.com/images" in u:
                        continue
                    if u not in found:
                        found.append(u)
            ...
            for u in found:
                try:
                    rr=requests.get(u,timeout=10,headers={"User-Agent":headers["User-Agent"]})
                    rr.raise_for_status()
            ...
            if not data:
                self.after(0,lambda:messagebox.showinfo(
                    "Google 후보 검색",
                    "Google에서 후보 이미지를 직접 가져오지 못했습니다.\n"
                    "'Google 이미지 검색' 버튼으로 같은 검색어의 Google 결과를 열 수 있습니다.\n\n"
                    "Google 페이지 구조나 자동 요청 제한에 따라 이 기능이 일시적으로 동작하지 않을 수 있습니다."
                ))
        except Exception as e:
